"""
CANVA Engine v20.0 — World-Class Editorial  ★ ADAPTIVE LAYOUT
"""
...
def _accent_dot(slide, T, x, y, size=0.45):
    """نقطة زخرفية مضيئة"""
    d = oval(slide, x, y, size, size, _rgb(T.accent))
    if d:
        gradient_fill(d, T.accent_grad1, T.accent_grad2, 45)
        glow(d, T.accent_grad2, radius_pt=5, alpha=0.4)
...
def make_plan(prs, req, T):
    ...
    for i,chap in enumerate(chapters):
        ...
        if tp: gradient_fill(tp,T.accent_grad1,T.accent_grad2,0)
        txt(slide,f"{i+1:02d}",x+0.14,y+0.06,2.0,0.5,
...
def make_objectives(prs, req, T):
    ...
    for i,(lbl,items) in enumerate(cols[:2]):
        ...
        txt_label(slide,lbl,x+0.2,cy+0.07,cw-0.4,0.52,
                  ...
                  align=PP_ALIGN.CENTER,rtl=True,uppercase=False)
        n_items = len(items)
        # ADAPTIVE: if few items, make them bigger and more spaced
        max_items = min(n_items, 8)
        ih = min((ch-0.7)/max(max_items,1), 1.52 if n_items >= 4 else 2.2)
...
def make_importance(prs, req, T):
    ...
    # ADAPTIVE layout: few items → fewer cols, bigger cards, larger text
    if n <= 2:
        cols = n; rows = 1
        body_size = 14.5
        num_size = 28
        ...
        cols = 3 if n >= 4 else 2; rows = (n+cols-1)//cols
        body_size = 11.5
        num_size = 21
...
def make_results(prs, req, T):
    ...
    # ADAPTIVE: few results get larger, more impactful treatment
    if n <= 3:
        ih = min(avail/n - 0.2, 3.8)
        body_size = 16
        pill_h = 0.7
        pill_w = 2.5
        pill_fsize = 14
    ...
    for i,res in enumerate(results):
        y=cy+i*(ih+0.14 if n <= 3 else 0.12)
        ...
        _pill(slide,T,W-3.2,y+(ih-pill_h)/2,pill_w,pill_h,str(i+1),pill_fsize)
        vline(slide,0.8,y,ih,_rgb(T.accent),thickness=vline_thick)
        # For few results, add a subtle decorative dot
        if n <= 3:
            _accent_dot(slide,T,1.1,y+(ih-0.45)/2)
        txt_body(slide,res,1.58 if n<=3 else 1.38,y,
                 W-5.5 if n<=3 else W-5.35,ih,
                 font=_FONT,size=body_size,color=_rgb(T.text_light),
...
def make_recommendations(prs, req, T):
    ...
    # ADAPTIVE sizing
    if n <= 3:
        ih = min(avail/n - 0.2, 3.6)
        body_size = 16
        tag_w = 0.58
    elif n <= 5:
        ih = min(avail/n - 0.15, 2.4)
        body_size = 14.5
        tag_w = 0.52
    else:
        ih = min(avail/n - 0.12, 1.52)
        body_size = 13.5
        tag_w = 0.46
    ...
    for i,rec in enumerate(recs):
        ...
        tg=rrect(slide,0.8,y,tag_w,ih,_rgb(T.accent),radius_pct=6)
        if tg: gradient_fill(tg,T.accent_grad1,T.accent_grad2,90)
        # Number badge for few recs
        if n <= 4:
            _pill(slide,T,W-3.0,y+(ih-0.54)/2,2.0,0.54,str(i+1),12)
            tw = _CW()-2.5-tag_w-0.2
        else:
            tw = _CW()-tag_w-0.2
        txt_body(slide,rec,0.8+tag_w+0.14,y+0.12,tw,ih-0.24,
                 font=_FONT,size=body_size,color=_rgb(T.text_light),
                 align=PP_ALIGN.RIGHT,rtl=True)
    return slide
...
def make_future(prs, req, T):
    ...
    if not items: return slide
    n=len(items)
    # ADAPTIVE columns: 2 items → 2 large cols, not 2 small cards
    ...
    # Larger numbers and text when fewer items
...
def make_references(prs, req, T):
    ...
    # ADAPTIVE sizing
    if n <= 4:
        ih = min(avail/n - 0.1, 2.8)
        body_size = 13
    elif n <= 8:
        ih = min(avail/n - 0.08, 1.6)
        body_size = 11.5
    else:
        ih = max(min(avail/n - 0.08, 1.02), 0.48)
        body_size = 10.5
    ...
    for i,ref in enumerate(refs):
        ...
        pill_w = min(len(f"[{i+1}]")*0.25+1.4, 2.2)
        _pill(slide,T,W-0.8-pill_w,y+(ih-0.44)/2,pill_w,0.44,f"[{i+1}]",9)
        txt_body(slide,ref,1.2,y+0.05,_CW()-pill_w-0.75,ih-0.1,
                 font=_FONT,size=body_size,color=_rgb(T.text_light),
                 align=PP_ALIGN.RIGHT,rtl=True)
    return slide
